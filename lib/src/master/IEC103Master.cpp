...
void IEC103Master::readGenericGroup(uint8_t deviceAddr, uint8_t group)
{
    ...
    m_genericState.asduAddr = asduAddr;
...
void IEC103Master::processAsdu8(const Asdu& asdu)
{
    ...
    if (parser.parse(asdu)) {
        ...
        bool addrMatch = (m_giState.asduAddr == 0) || 
                         (m_giState.asduAddr == parser.result().asduAddr);
    ...
    if (parser.parse(asdu)) {
        ...
        for (const auto& item : parser.data().items) {
            ...
            if (m_handler) {
                ...
                m_handler->onGenericData(asduAddr, item);
            }
        }
    }
}
