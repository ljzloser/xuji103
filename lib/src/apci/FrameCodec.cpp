...
namespace IEC103
{
    ...
    Frame::Frame(const QByteArray &data) : m_apci{}, m_asdu{}, m_valid(false)
    {
        // 南网规范帧格式 (参照104标准):
    ...
    Frame::Frame(const APCI &apci, const QByteArray &asdu)
        : m_apci(apci), m_asdu(asdu), m_valid(true) {}
    ...
    uint16_t Frame::sendSeq() const
    {
        if (type() == ApduType::I_FORMAT)
        {
            ...
            // byte2: N(S)高8位
            // N(S) = (byte2 << 7) | (byte1 >> 1)
            return (static_cast<uint16_t>(m_apci.byte2) << 7) |
                   (static_cast<uint16_t>(m_apci.byte1) >> 1);
        }
        return 0;
    }
    ...
    uint16_t Frame::recvSeq() const
    {
        ...
        {
            // IEC 104 序号格式:
            // byte3: N(R)低7位左移1位，bit0=0
            // byte4: N(R)高8位
            // N(R) = (byte4 << 7) | (byte3 >> 1)
            return (static_cast<uint16_t>(m_apci.byte4) << 7) |
                   (static_cast<uint16_t>(m_apci.byte3) >> 1);
        }
    ...
    void Frame::setSendSeq(uint16_t seq)
    {
        ...
        // byte1: N(S)低7位左移1位，bit0=0
        // byte2: N(S)高8位
        m_apci.byte1 = static_cast<uint8_t>((seq << 1) & 0xFF);
        m_apci.byte2 = static_cast<uint8_t>((seq >> 7) & 0xFF);
    }
    ...
    void Frame::setRecvSeq(uint16_t seq)
    {
        ...
        // byte3: N(R)低7位左移1位，bit0=0
        // byte4: N(R)高8位
        m_apci.byte3 = static_cast<uint8_t>((seq << 1) & 0xFF);
        m_apci.byte4 = static_cast<uint8_t>((seq >> 7) & 0xFF);
    }

    void Frame::setSFormat(uint16_t recvSeqVal)
    {
        m_apci.byte1 = 0x01; // S格式标识
        m_apci.byte2 = 0x00;
        // N(R) 编码同 I 格式
        m_apci.byte3 = static_cast<uint8_t>((recvSeqVal << 1) & 0xFF);
        m_apci.byte4 = static_cast<uint8_t>((recvSeqVal >> 7) & 0xFF);
    }
    ...
    bool FrameCodec::verifyChecksum(const uint8_t *data, size_t len)
    {
        ...
        // 结束字节位置 = 4 + frameLen
        if (len < static_cast<size_t>(5 + frameLen))
            return false;
    ...
    {
        if (!data || len < 7)
            return false; // 最小帧: 68H + L(2) + APCI(4) = 7字节
